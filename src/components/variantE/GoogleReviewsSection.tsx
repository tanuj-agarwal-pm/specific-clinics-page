  ...
  return <section className="py-16 md:py-24 px-4 bg-background">
       <div className="container mx-auto max-w-7xl">
        ...
        <div ref={scrollContainerRef} className="flex gap-4 overflow-x-auto pb-4 snap-x snap-mandatory -mx-4 px-4 md:mx-0 md:px-0 md:grid md:grid-cols-2 lg:grid-cols-4 md:gap-6 md:overflow-visible md:pb-0 scrollbar-hide">
          ...
          const displayText = shouldTruncate && !isExpanded ? review.text.slice(0, 120) + "..." : review.text;
          return <div key={index} className="bg-card rounded-xl p-5 md:p-6 shadow-sm border border-border flex flex-col min-w-[280px] w-[80vw] flex-shrink-0 snap-center md:min-w-0 md:w-auto md:flex-shrink">
                 {/* Star Rating */}
                 <div className="flex gap-0.5 mb-3">
                   {Array.from({
                length: 5
              }).map((_, i) => <Star key={i} className={`w-5 h-5 ${i < review.rating ? "fill-yellow-400 text-yellow-400" : "fill-muted text-muted"}`} />)}
                 </div>
 
                 {/* Review Text */}
                 <p className="text-foreground text-sm leading-relaxed mb-4 flex-1">
                   {displayText}
                   {shouldTruncate && <button onClick={() => setExpandedReview(isExpanded ? null : index)} className="text-foreground font-semibold underline ml-1 hover:text-primary transition-colors">
        ...
        <div className="flex justify-center gap-2 mt-4 md:hidden">
          {reviews.map((_, index) => <button key={index} onClick={() => scrollToIndex(index)} className={`w-2 h-2 rounded-full transition-all ${activeIndex === index ? "bg-primary w-6" : "bg-muted-foreground/30"}`} aria-label={`Go to review ${index + 1}`} />)}
        </div>
       </div>
     </section>;
};